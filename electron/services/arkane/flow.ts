...
export class AuthStateEmitter extends EventEmitter {
  static ON_ARKANE_TOKEN_RESPONSE = 'on_token_response';
}
...
const openIdConnectUrl = environmentService.getVenlyLogin();


/* example client configuration */
const clientId = 'Gaimin';
...
export class AuthFlow {
  ...
  constructor() {
    console.log('AuthFlow arkane constructor start');
    ...
    this.notifier.setAuthorizationListener((request, response, error) => {
      ...
      if (response) {
        ...
        this.makeRefreshTokenRequest(response.code, codeVerifier)
          ...
          .then(() => {
            this.authStateEmitter.emit(AuthStateEmitter.ON_ARKANE_TOKEN_RESPONSE);
            log('All Done.');
          });
      }
    });
  ...
  private makeRefreshTokenRequest(code: string, codeVerifier: string | undefined): Promise<void> {
    ...
    return this.tokenHandler
      ...
      .then(response => {
        log(`Arkane refresh Token is ${response.refreshToken}`);
        this.refreshToken = response.refreshToken;
        this.accessTokenResponse = response;
        log(`Arkane access Token is ${response.accessToken}`);
        return response;
      })
      .then(() => {
      });
  ...
  performWithFreshTokens(): Promise<TokenResponse> {
    if (!this.configuration) {
      log('Unknown service configuration');
    ...
    return this.tokenHandler
      .performTokenRequest(this.configuration, request)
      .then(response => {
        console.log('Arkane access Token: ' + response.accessToken);
        console.log('Arkane refresh Token: ' + response.refreshToken);
